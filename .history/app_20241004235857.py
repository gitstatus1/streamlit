...
st.markdown("<style>.subtitle{font-size:18px !important;}</style>", unsafe_allow_html=True)
...
with st.container():
    ...
    with mid:
        st.markdown('<p class="subtitle">Summary</p>', unsafe_allow_html= True)
...
with st.container():
    ...
    _, mid,  _ = st.columns(3)
    with mid:
        st.markdown('<p class="subtitle">Project</p>', unsafe_allow_html= True)
        st.write("##")
