...
with st.container(border= True): #key= "Bollinger Bands"
    ...
    with left_col:
        ...
        st.markdown('''
                    <p class="text">
                    This project is focused on implementing a trading strategy using Bollinger Bands for a specific stock ("pls.ax") over a minute-by-minute interval. The code calculates the upper and lower Bollinger Bands by using a moving average and standard deviation of stock prices over a 20-minute window. Based on these bands, trading signals are generated: a buy signal when the price is below the lower band, and a sell signal when the price exceeds the upper band. The program then executes trades based on the generated signals, tracking the portfolio's performance and profit over time. It concludes by visualizing the stock's price along with the calculated Bollinger Bands. 
                    </p>
                    ''',
                      unsafe_allow_html=True)
        st.write("##")
