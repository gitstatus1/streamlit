...
with st.container(): #key = "top container"
    ...
    with col2:
        st_lottie(robot_gif, key = 'robot', height = 400, width = 300)
...
with st.container(): #key = Image Generation Project
    ...
    with left_col:
        ...
        st.write("##")
        st.write("[✨See Project Link✨]()")
                    
                    <a href="https://colab.research.google.com/drive/1MNslw2EyoMf1QSsPnBT_FaGDAYhRBbkD?usp=sharing"><p class="text">✨See Project Link✨ </p></a>
                   ''')
...
with st.container(): #key = Contacts
    ...
    _, mid,  _ = st.columns(3)
    with mid:
        st.markdown('<p class="subtitle">Contacts</p>', unsafe_allow_html= True)
